...
class Keyboard(QWidget):
	...
	def readKeyboards(self, kbdfiles):
		wasempty = True if self.kbds == [] else False
		for kbdfile in kbdfiles:
			if os.access(kbdfile, os.R_OK):
				with open(kbdfile, 'r', encoding='utf-8') as f:
					self.kbds.append(json.load(f))
			elif pkg_resources.resource_exists('oskb', 'keyboards/' + kbdfile):
				self.kbds.append(json.loads(pkg_resources.resource_string('oskb', 'keyboards/' + kbdfile)))
			else:
