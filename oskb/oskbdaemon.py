...
def main():
	...
	ap.add_argument('--keypipe', '-p', help='filename for keypipe', metavar='<filename>', default='/var/run/oskb-keypipe')
	ap.add_argument('--debug', help='logs every keystroke to log (UNSAFE!)', action='store_true')
	...
	try:
		...
		log.critical('Error: user or group name not found, exiting.')
		sys.exit(-1)
	...
	try:
		...
		log.critical('Error: permission denied when creating keypipe. (Not root?)')
		sys.exit(-2)
	except:
		log.critical('Error: Could not create named pipe for keyboard stream.')
		sys.exit(-3)
	...
	try:
		...
		log.critical('Error: Could not set up keyboard injection, exiting.')
		sys.exit(-4)
	...
	try:
		...
		log.critical('Error: Could not drop privs after setting up keyboard injection.')
		sys.exit(-5)
	...
	try:
		...
		log.critical("Runtime exception: {0}".format(log_msg))
		sys.exit(-6)
